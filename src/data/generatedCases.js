...
function generatedCustomerRelationshipProfile({ id, accounts, reportedDate }) {
  ...
  const hasDepositProduct = productKinds.some((kind) => /checking|savings|deposit/.test(kind));
  const hasSpendingProduct = productKinds.some((kind) => /card|checking/.test(kind));
  return {
    sourceRecordId: `${id}-C360-RELATIONSHIP`,
    asOf: reportedDate,
    normalDeposits: hasDepositProduct
      ? 'A deposit-capable product is recorded; dated activity remains in Financial Investigation.'
      : 'No deposit-capable product is recorded in this relationship snapshot.',
    normalSpending: hasSpendingProduct
      ? 'A spending-capable product is recorded; dated activity remains in Financial Investigation.'
      : 'No spending-capable product is recorded in this relationship snapshot.',
    authorizedUsers: 'No additional authorized user is included in the generated relationship packet.',
    digitalBanking: 'A digital relationship profile is recorded; authentication records remain separate.',
  };
}
...
export function createGeneratedCase(indexOrOptions = Date.now(), options = {}) {
  ...
  customer.relationshipPacketVersion = 1;
  customer.identity = generatedCustomerIdentitySnapshot({
    id,
    index,
    person,
    city,
    ...
    profileChanges,
    accounts: toolResults.relationshipAccounts,
  });
  customer.serviceContacts = generatedCustomerServiceContacts({
    id,
    person,
    profileChanges,
    accounts: toolResults.relationshipAccounts,
  ...
  customer.relationshipProfile = generatedCustomerRelationshipProfile({
    id,
    accounts: toolResults.relationshipAccounts,
    reportedDate,
  });
